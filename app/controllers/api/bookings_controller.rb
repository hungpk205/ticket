...
class Api::BookingsController < ApplicationController
  ...
  def index
    @bookings = Booking.where(fullname: params[:fullname], identity_card: params[:identity_card], phone: params[:phone])
    if @bookings.size > 0
      @result = Array.new
      @bookings.each do |booking|
        ...
        @booking_json = BookingJson.new(booking.id, booking.trip.id, booking.trip.start_time, booking.trip.route.start_place, booking.trip.route.end_place, @employees, price, booking.status, booking.trip.bus.license_plate, booking.created_at)
        @result << @booking_json
      end
      render json: @result
    else
  ...
  def show
    ...
    if @booking.present?
      if @booking.fullname == params[:fullname] && @booking.identity_card == params[:identity_card] && @booking.phone == params[:phone]
